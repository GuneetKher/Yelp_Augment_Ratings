...
def main(inputs,model_file):
    ...
    predictions = predictions.withColumn('prediction',functions.when(predictions['prediction']>5,5).when(predictions['prediction']<1,1).otherwise(predictions['prediction']))
    predictions = predictions.withColumn('new_prediction',functions.round(predictions['prediction']))
